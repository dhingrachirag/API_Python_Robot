...
*** Keywords ***
Create initial Gateway setup for O2_UK
    Suppress Insecure Request Warnings
    Create IPSec gateway with 1 Port and 1 Link Linode O2_UK
    Get All IPSEC gateways and fetch newly created GatewayID O2_UK
    Check Disable status of Gateway O2_UK
    Patch Ipsec Gateways for O2_UK Linode
    Check online Status after enabling the Gateway O2_UK
    Attach to Default Network O2_UK

Tear down initial Gateway Setup for O2_UK
    Delete Gateway Attachment O2_UK
    Disable Ipsec Gateway O2_UK
    Check disabled Status after disabling the Gateway O2_UK
    Delete Ipsec Gateway O2_UK

Create IPSec gateway with 1 Port and 1 Link Linode O2_UK
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside O2_UK
    ...
    IF    "${gateway_id}" == ""
           ${response}=    call post request    ${headers2}    ${base_url}   ${ipsec_gateways}    ${IPsec_gateway_1port_1link_O2_UK}
           log    ${response}
...
Get All IPSEC gateways and fetch newly created GatewayID O2_UK
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${O2_UK_automation}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${O2_UK_automation}'
...
Check Disable status of Gateway O2_UK
   ${status_gateway}=    Get Link Status of gateway for O2_UK Gateway
...
Patch Ipsec Gateways for O2_UK Linode
    ...
    ${X-Asavie-Account-Id}=    Get Account ID Inside O2_UK
    log    ${X-Asavie-Account-Id}
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID O2_UK
    log    ${new_gateway_id}
...
Check Online Status after enabling the Gateway O2_UK
   ${status_gateway}=    Get Link Status of gateway for O2_UK Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check online status O2_UK

Check online status O2_UK
   ${status_gateway}=    Get Link Status of gateway for O2_UK Gateway
   log    ${status_gateway}
...
Attach to Default Network O2_UK
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID O2_UK
    log    ${new_gateway_id}
    ${gateway_O2_UK_network}=    set variable    ${ipsec_gateways}/${new_gateway_id}/attachments
    log    ${gateway_O2_UK_network}
    ${token1}=       Set Variable    Bearer ${token}
    ...
    ${response}=    call post request    ${headers2}    ${base_url}    ${gateway_O2_UK_network}    ${attach_production_network_O2_UK}
    log    ${response}
    Should Be True     ${response.status_code} == 201
    ${gateway_id}    get value from json     ${response.json()}    id
    log    ${gateway_id}
...
Get Link Status of gateway for O2_UK Gateway
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside O2_UK
    log    ${X-Asavie-Account-Id}
...
Get Account ID Inside O2_UK
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
    ${X-Asavie-Tenant-Id}=    Get O2_UK Tenant ID for Production Environment
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           ${acc_name_str}=   Evaluate             "".join(${acc_name})
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${O2_UK_Account_Tenant}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${O2_UK_Account_Tenant}'
           ${success_num}=    Evaluate    ${success_num} + 1
           log    ${success_num}
    END
    ${convertListToString}=   Evaluate             "".join(${var})
...
Get O2_UK Tenant ID for Production Environment
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
...
Delete Gateway Attachment O2_UK
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID O2_UK
    log    ${new_gateway_id}
    ${attachment_id}=    Get Attachment ID for newly created Gateway O2_UK
    log    ${attachment_id}
...
Check disabled Status after disabling the Gateway O2_UK
   ${status_gateway}=    Get Link Status of gateway for O2_UK Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check disabled status O2_UK

Check disabled status O2_UK
   ${status_gateway}=    Get Link Status of gateway for O2_UK Gateway
   log    ${status_gateway}
   should be equal    ${status_gateway}    ${disable}    msg=Status of Ipsec coming wrong.Possibility of bug.
...
Get Attachment ID for newly created Gateway O2_UK
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside O2_UK
    log    ${X-Asavie-Account-Id}
