...
*** Keywords ***
Create initial Gateway setup for Vodafone UK
    Suppress Insecure Request Warnings
    Create IPSec gateway with 1 Port and 1 Link Linode Vodafone UK
    Get All IPSEC gateways and fetch newly created GatewayID Vodafone UK
    Check Disable status of Gateway Vodafone UK
    Patch Ipsec Gateways for Vodafone UK Linode
    Check online Status after enabling the Gateway Vodafone UK
#    Attach to Default Network Vodafone UK

Tear down initial Gateway Setup for Vodafone UK
    Delete Gateway Attachment Vodafone UK
    Disable Ipsec Gateway Vodafone UK
    Check disabled Status after disabling the Gateway Vodafone UK
    Delete Ipsec Gateway Vodafone UK

Create IPSec gateway with 1 Port and 1 Link Linode Vodafone UK
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone UK
    ...
    IF    "${gateway_id}" == ""
           ${response}=    call post request    ${headers2}    ${base_url}   ${ipsec_gateways}    ${IPsec_gateway_1port_1link_Vodafone_UK}
           log    ${response}
...
Get All IPSEC gateways and fetch newly created GatewayID Vodafone UK
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${Vodafone_UK_automation}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${Vodafone_UK_automation}'
...
Check Disable status of Gateway Vodafone UK
   ${status_gateway}=    Get Link Status of gateway for Vodafone UK Gateway
...
Patch Ipsec Gateways for Vodafone UK Linode
    ...
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone UK
    log    ${X-Asavie-Account-Id}
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID Vodafone UK
    log    ${new_gateway_id}
...
Check Online Status after enabling the Gateway Vodafone UK
   ${status_gateway}=    Get Link Status of gateway for Vodafone UK Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check online status Vodafone UK

Check online status Vodafone UK
   ${status_gateway}=    Get Link Status of gateway for Vodafone UK Gateway
   log    ${status_gateway}
...
Attach to Default Network Vodafone UK
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID Vodafone UK
    log    ${new_gateway_id}
    ${gateway_vodafone_network}=    set variable    ${ipsec_gateways}/${new_gateway_id}/attachments
    log    ${gateway_vodafone_network}
    ${token1}=       Set Variable    Bearer ${token}
    ...
    ${response}=    call post request    ${headers2}    ${base_url}    ${gateway_vodafone_network}    ${attach_production_network_Vodafone_UK}
    log    ${response}
    Should Be True     ${response.status_code} == 201
    ${gateway_id}    get value from json     ${response.json()}    id
    log    ${gateway_id}
...
Get Link Status of gateway for Vodafone UK Gateway
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone UK
    log    ${X-Asavie-Account-Id}
...
Get Account ID Inside Vodafone UK
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
    ${X-Asavie-Tenant-Id}=    Get Vodafone UK Tenant ID for Production Environment
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           ${acc_name_str}=   Evaluate             "".join(${acc_name})
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${Vodafone UK_Account_Tenant}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${Vodafone UK_Account_Tenant}'
           ${success_num}=    Evaluate    ${success_num} + 1
           log    ${success_num}
    END
    ${convertListToString}=   Evaluate             "".join(${var})
...
Get Vodafone UK Tenant ID for Production Environment
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           ${acc_name_str}=   Evaluate             "".join(${acc_name})
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${Vodafone_UK_Account_Tenant}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${Vodafone_UK_Account_Tenant}'
           ${success_num}=    Evaluate    ${success_num} + 1
           log    ${success_num}
    END
    ${convertListToString}=   Evaluate             "".join(${var})
...
Delete Gateway Attachment Vodafone UK
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID Vodafone UK
    log    ${new_gateway_id}
    ${attachment_id}=    Get Attachment ID for newly created Gateway Vodafone
    log    ${attachment_id}
...
Check disabled Status after disabling the Gateway Vodafone UK
   ${status_gateway}=    Get Link Status of gateway for Vodafone UK Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check disabled status Vodafone UK

Check disabled status Vodafone UK
   ${status_gateway}=    Get Link Status of gateway for Vodafone UK Gateway
   log    ${status_gateway}
   should be equal    ${status_gateway}    ${disable}    msg=Status of Ipsec coming wrong.Possibility of bug.
...
Get Attachment ID for newly created Gateway Vodafone
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone UK
    log    ${X-Asavie-Account-Id}
