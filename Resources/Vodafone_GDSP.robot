...
*** Keywords ***
Create initial Gateway setup for Vodafone GDSP
    Suppress Insecure Request Warnings
    Create IPSec gateway with 1 Port and 1 Link Linode Vodafone GDSP
    Get All IPSEC gateways and fetch newly created GatewayID Vodafone GDSP
    Check Disable status of Gateway Vodafone GDSP
    Patch Ipsec Gateways for Vodafone GDSP Linode
    Check online Status after enabling the Gateway Vodafone GDSP
    Attach to Default Network Vodafone GDSP

Tear down initial Gateway Setup for Vodafone GDSP
    Delete Gateway Attachment Vodafone GDSP
    Disable Ipsec Gateway Vodafone GDSP
    Check disabled Status after disabling the Gateway Vodafone GDSP
    Delete Ipsec Gateway Vodafone GDSP

Create IPSec gateway with 1 Port and 1 Link Linode Vodafone GDSP
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone GDSP
    ...
    IF    "${gateway_id}" == ""
           ${response}=    call post request    ${headers2}    ${base_url}   ${ipsec_gateways}    ${IPsec_gateway_1port_1link_Vodafone GDSP}
           log    ${response}
...
Get All IPSEC gateways and fetch newly created GatewayID Vodafone GDSP
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${Vodafone_automation}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${Vodafone_automation}'
...
Check Disable status of Gateway Vodafone GDSP
   ${status_gateway}=    Get Link Status of gateway for Vodafone GDSP Gateway
...
Patch Ipsec Gateways for Vodafone GDSP Linode
    ...
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone GDSP
    log    ${X-Asavie-Account-Id}
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID Vodafone GDSP
    log    ${new_gateway_id}
...
Check Online Status after enabling the Gateway Vodafone GDSP
   ${status_gateway}=    Get Link Status of gateway for Vodafone GDSP Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check online status Vodafone GDSP

Check online status Vodafone GDSP
   ${status_gateway}=    Get Link Status of gateway for Vodafone GDSP Gateway
   log    ${status_gateway}
...
Attach to Default Network Vodafone GDSP
    ...
    ${response}=    call post request    ${headers2}    ${base_url}    ${gateway_vodafone_network}    ${attach_production_network_Vodafone_GDSP}
    log    ${response}
    Should Be True     ${response.status_code} == 201
    ${gateway_id}    get value from json     ${response.json()}    id
    log    ${gateway_id}
...
Get Link Status of gateway for Vodafone GDSP Gateway
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside Vodafone GDSP
    log    ${X-Asavie-Account-Id}
...
Get Account ID Inside Vodafone GDSP
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
    ${X-Asavie-Tenant-Id}=    Get Vodafone GDSP Tenant ID for Production Environment
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           ${acc_name_str}=   Evaluate             "".join(${acc_name})
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${Vodafone GDSP_Account_Tenant}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${Vodafone GDSP_Account_Tenant}'
           ${success_num}=    Evaluate    ${success_num} + 1
           log    ${success_num}
    END
    ${convertListToString}=   Evaluate             "".join(${var})
...
Get Vodafone GDSP Tenant ID for Production Environment
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
...
Check disabled Status after disabling the Gateway Vodafone GDSP
   ${status_gateway}=    Get Link Status of gateway for Vodafone GDSP Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check disabled status Vodafone GDSP

Check disabled status Vodafone GDSP
   ${status_gateway}=    Get Link Status of gateway for Vodafone GDSP Gateway
   log    ${status_gateway}
   should be equal    ${status_gateway}    ${disable}    msg=Status of Ipsec coming wrong.Possibility of bug.
