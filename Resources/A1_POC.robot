...
*** Keywords ***
Create initial Gateway setup for A1-POC
    Suppress Insecure Request Warnings
    Create IPSec gateway with 1 Port and 1 Link Linode A1-POC
    Get All IPSEC gateways and fetch newly created GatewayID A1-POC
    Check Disable status of Gateway A1-POC
    Patch Ipsec Gateways for A1-POC Japser Linode
    Check online Status after enabling the Gateway A1-POC
#    Attach to Default Network A1-POC

Tear down initial Gateway Setup for A1-POC
    Delete Gateway attachment A1-POC
    Disable Ipsec Gateway A1-POC
    Check disabled Status after disabling the Gateway A1-POC
    Delete Ipsec Gateway A1-POC

Create IPSec gateway with 1 Port and 1 Link Linode A1-POC
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside A1-POC
    ...
    IF    "${gateway_id}" == ""
           ${response}=    call post request    ${headers2}    ${base_url}   ${ipsec_gateways}    ${IPsec_gateway_1port_1link_A1_POC}
           log    ${response}
...
Get All IPSEC gateways and fetch newly created GatewayID A1-POC
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${A1-POC_automation}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${A1-POC_automation}'
...
Check Disable status of Gateway A1-POC
   ${status_gateway}=    Get Link Status of gateway for A1-POC Gateway
...
Patch Ipsec Gateways for A1-POC Japser Linode
    ...
    ${X-Asavie-Account-Id}=    Get Account ID Inside A1-POC
    log    ${X-Asavie-Account-Id}
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID A1-POC
    log    ${new_gateway_id}
...
Check Online Status after enabling the Gateway A1-POC
   ${status_gateway}=    Get Link Status of gateway for A1-POC Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check online status A1-POC

Check online status A1-POC
   ${status_gateway}=    Get Link Status of gateway for A1-POC Gateway
   log    ${status_gateway}
...
Attach to Default Network A1-POC
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID A1-POC
    log    ${new_gateway_id}
    ${gateway_A1-POC_network}=    set variable    ${ipsec_gateways}/${new_gateway_id}/attachments
    log    ${gateway_A1-POC_network}
    ${token1}=       Set Variable    Bearer ${token}
    ...
    ${response}=    call post request    ${headers2}    ${base_url}    ${gateway_A1-POC_network}    ${attach_production_network_A1_POC}
    log    ${response}
    Should Be True     ${response.status_code} == 201
    ${gateway_id}    get value from json     ${response.json()}    id
    log    ${gateway_id}
...
Get Link Status of gateway for A1-POC Gateway
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside A1-POC
    log    ${X-Asavie-Account-Id}
...
Get Account ID Inside A1-POC
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
    ${X-Asavie-Tenant-Id}=    Get A1-POC Tenant ID for Production Environment
    ...
    FOR    ${item}    IN RANGE    ${success_num}    ${length}
           ...
           ${acc_name_str}=   Evaluate             "".join(${acc_name})
           log    ${acc_name_str}
           ${var}=    set variable if    '${acc_name_str}' == '${A1-POC_Account_Tenant}'    ${acc_id}
           exit for loop if    '${acc_name_str}' == '${A1-POC_Account_Tenant}'
           ${success_num}=    Evaluate    ${success_num} + 1
           log    ${success_num}
    END
    ${convertListToString}=   Evaluate             "".join(${var})
...
Get A1-POC Tenant ID for Production Environment
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Owner-Id}=    GET Owner Tenant ID for Production Environment
    log    ${X-Asavie-Owner-Id}
...
Delete Gateway attachment A1-POC
    ...
    ${new_gateway_id}=    Get All IPSEC gateways and fetch newly created GatewayID A1-POC
    log    ${new_gateway_id}
    ${attachment_id}=    Get Attachment ID for newly created Gateway A1-POC
    log    ${attachment_id}
    ${delete_A1-POCattach}=    set variable    ${ipsec_gateways}/${new_gateway_id}/attachments/${attachment_id}
    log    ${delete_A1-POCattach}
    ${token1}=       Set Variable    Bearer ${token}
    set global variable    &{headers2}     Authorization=${token1}    Accept=${aster}    X-Asavie-Account-Id=${X-Asavie-Account-Id}
    ${response}=   call delete request    ${headers2}    ${base_url}   ${delete_A1-POCattach}
    log    ${response}
...
Check disabled Status after disabling the Gateway A1-POC
   ${status_gateway}=    Get Link Status of gateway for A1-POC Gateway
   log    ${status_gateway}
   wait until keyword succeeds    5min    30s    Check disabled status A1-POC

Check disabled status A1-POC
   ${status_gateway}=    Get Link Status of gateway for A1-POC Gateway
   log    ${status_gateway}
   should be equal    ${status_gateway}    ${disable}    msg=Status of Ipsec coming wrong.Possibility of bug.
...
Get Attachment ID for newly created Gateway A1-POC
    ${token}=    Configure New Token for Production environment
    log    ${token}
    ${X-Asavie-Account-Id}=    Get Account ID Inside A1-POC
    log    ${X-Asavie-Account-Id}
